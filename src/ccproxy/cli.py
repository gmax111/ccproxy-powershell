...
# Cross-platform process utilities for Windows compatibility
def _process_exists(pid: int) -> bool:
    """Check if a process exists (cross-platform).

    On Windows, os.kill(pid, 0) doesn't work reliably, so we use ctypes
    to check process existence via OpenProcess.
    """
...
def _terminate_process(pid: int, force: bool = False) -> bool:
    ...
    On Windows, uses taskkill. On Unix, uses SIGTERM/SIGKILL signals.
